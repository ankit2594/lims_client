        ...
        return(
            ...
            <div className="card review-card" style={{backgroundColor:"rgb(255, 248, 220)",marginLeft:"7%", marginRight:"5%"}}>  
                ...
                {this.state.review?cardReview:""}
                            {reviewData}
                            {this.props.reviews!==null && this.props.reviews.length>5?<div> {this.state.showdown?<div onClick={this.loadMore} style={{color:'#614126', fontSize:'30px'}} className="fa fa-sort-down"></div>:<div onClick={this.showLess} style={{color:'#614126', fontSize:'30px'}} className="fa fa-sort-up"></div>}</div>:null}
                            </div> 
            </div>
        )
    }
